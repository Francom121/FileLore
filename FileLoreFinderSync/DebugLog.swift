...
/// File-based diagnostic logger for the Finder Sync extension.
///
/// Unified logging redacts `NSLog` output from the ExtensionKit-hosted
/// extension (everything shows as `<private>`), which leaves badge debugging
/// blind. This writes plain timestamped lines to a text file inside the
/// extension's sandbox container instead: in this sandboxed process
/// `homeDirectoryForCurrentUser` IS the container's `Data` directory
/// (`~/Library/Containers/com.filelore.app.FinderSync/Data`), which is always
/// writable.
///
...
enum DebugLog {
    ...
    static let fileURL: URL = {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("filelore-badge-debug.log", isDirectory: false)
    }()

    /// The log is rolled once it exceeds this many bytes.
    private static let maxBytes: UInt64 = 256 * 1024

    private static let queue = DispatchQueue(label: "com.filelore.app.FinderSync.DebugLog")
