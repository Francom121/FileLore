...
/// even while FileLore is in the background, which is the whole point: press
...
final class HotKeyManager {
    ...
    /// Hotkey signature: 'FLOR'.
    private static let signature: OSType = 0x464C4F52
    ...
    func register() {
        ...
        guard installStatus == noErr else {
            NSLog("FileLore: InstallEventHandler failed (\(installStatus))")
            return
        }
        ...
        for slot in HotKeySlot.allCases {
            ...
            if registerStatus == noErr, let ref {
                hotKeyRefs[slot] = ref
                anyRegistered = true
                NSLog("FileLore: \(shortcut.displayString) global hotkey registered (\(slot.rawValue))")
            } else {
                NSLog("FileLore: RegisterEventHotKey failed (\(registerStatus)) — another app may already own \(shortcut.displayString)")
            }
        }
