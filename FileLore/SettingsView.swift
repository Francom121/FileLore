...
/// Settings window: launch-at-login plus the global hotkeys — "open note for
/// the Finder selection" (default ⌥T) and "search notes" (default ⇧⌘F).
struct SettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Toggle("Launch FileLore at login", isOn: $launchAtLogin)
            ...
            Text("You can also assign any shortcut via System Settings → Keyboard → Keyboard Shortcuts → Services → Open FileLore Note.")
                .font(.caption)
        ...
        .frame(width: 400)
        .onAppear {
            // Reflect the real login-item status (it can change outside the app).
            launchAtLogin = SMAppService.mainApp.status == .enabled
        }
    }

    /// Registers/unregisters the app as a login item via SMAppService. The app
    /// is unsandboxed, so no entitlements or helper bundle are needed.
