...
/// Finder Sync "Batch Tag with FileLore…" context menu item (which hands the
/// file list over through a `filelore://batch?ref=` URL — see
/// `BatchRequestLoader`).
...
// MARK: - filelore://batch hand-off from the Finder Sync extension
...
/// JSON into its container's tmp directory and opens `filelore://batch?ref=<file>`.
...
enum BatchRequestLoader {
    ...
    private static var extensionTmpURL: URL {
        URL(fileURLWithPath: "/Users/\(NSUserName())", isDirectory: true)
            .appendingPathComponent("Library/Containers/com.filelore.app.FinderSync/Data/tmp", isDirectory: true)
    }

    /// Parses `filelore://batch?ref=<filename>` and loads the referenced path
    /// list. Returns `nil` for malformed URLs, missing files, or bad JSON.
    static func fileURLs(from url: URL) -> [URL]? {
        guard url.scheme == "filelore", url.host == "batch",
