...
/// Small local registry of files that carry a FileLore note.
///
/// Persisted at `~/Library/Application Support/FileLore/known-files.json`
/// (migrated from the Tether-era directory on first launch — see AppDelegate).
...
final class KnownFilesRegistry {
    ...
    private func persist() {
        do {
            ...
            NSLog("FileLore: failed to persist known-files.json: \(error.localizedDescription)")
        }
    }
